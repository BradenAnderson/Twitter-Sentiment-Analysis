...
This simple model suffers from several drawbacks (e.g. counting word frequencies along does not take into contextual information such as word ordering) but it is enough to illustrate one important point that holds true even for more complex methods. As the size of the vocabulary increases, the dimensionality of the feature’s matrix increases which causes the amount of training data that is required for a model to accurately generalize increases greatly. Furthermore, as the vocabulary increases the memory and computational resources required to process the training matrix increases. Lastly, as the size of the vocabulary becomes increasing large compared to the average document length (which is relatively short for tweets) the features matrix becomes increasingly sparse, and we may find that for words that occur the most infrequently there is simply not enough training data to accurately learn how the word impacts the desired target. 
